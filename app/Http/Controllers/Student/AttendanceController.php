...
{
    // Lấy thông tin user hiện tại từ Token (Sanctum)
    $user = $request->user();

    // Chuẩn bị dữ liệu nội dung QR
    // Lưu ý: Đảm bảo bảng users của bạn có cột 'mssv' hoặc tương đương
    $content = json_encode([
        'mssv' => $user->mssv ?? 'N/A', 
        'name' => $user->name,
        'type' => 'student_id',
    ...
    // Tạo QR Code và trả về định dạng SVG
